...
model = dict(
    roi_head=dict(  # RoIHead 封装了两步(two-stage)/级联(cascade)检测器的第二步。
        type='StandardRoIHead',  # RoI head 的类型，这是我自己定义的ROI head。更多细节请参考 https://github.com/open-mmlab/mmdetection/blob/master/mmdet/models/roi_heads/standard_roi_head.py#L10。
...
# 修改数据集相关设置
dataset_type = 'ICDAR_2017_STR_Dataset'
classes = ('cell',)
data = dict(
    train=dict(
        type=dataset_type,
        img_prefix='data/ICDAR2017_STR/Images/TrainSet',
        classes=classes,
        ann_file='data/ICDAR2017_STR/Annotations/TrainSet/train.json'),
    val=dict(
        type=dataset_type,
        img_prefix='data/ICDAR2017_STR/Images/TestSet',
        classes=classes,
        ann_file='data/ICDAR2017_STR/Annotations/TestSet/test.json'),
    test=dict(
        type=dataset_type,
        img_prefix='data/ICDAR2017_STR/Images/TestSet',
        classes=classes,
